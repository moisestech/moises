...
import { genaiResumePrint } from '@/content/genaiResumePrint';
import { resumeData } from '@/constants/resume';

export const metadata: Metadata = {
  title: 'Résumé (print) | Moises Sanabria — Senior GenAI Engineer',
  description: 'Print-friendly résumé for Senior GenAI Engineer recruiting dossier.',
  robots: { index: false, follow: false },
};

export default function GenaiResumePrintPage() {
  const p = genaiResumePrint;
  return (
    <>
      <style>{`
        @page { margin: 0.6in; }
      ...
      <div className="min-h-screen bg-white px-6 py-8 text-stone-900 print:px-0 print:py-0">
        <p className="no-print mb-6 text-center text-sm text-stone-500">
          ...
          <code className="rounded bg-stone-100 px-1">Moises-Sanabria-Senior-GenAI-Engineer.pdf</code>
        </p>
        <article className="mx-auto max-w-[720px] text-sm leading-relaxed">
          <header className="border-b border-stone-300 pb-4">
            ...
            <p className="mt-3 text-stone-700">
              ...
              {p.contact.github ? ` · ${p.contact.github}` : ''}
            </p>
          </header>
          ...
          <section className="mt-5">
            ...
            <ul className="mt-3 space-y-4">
              {resumeData.experience.slice(0, 6).map((job) => (
                <li key={`${job.company}-${job.period}`}>
                  ...
                  <ul className="mt-1 list-disc pl-4 text-stone-700">
                    {job.description.slice(0, 2).map((line) => (
