...
export type ContextStudyVisual = {
  id: string;
  title: string;
  strategicRole: string;
  description: string;
  images: readonly MachineSentenceMediaAsset[];
  notes: readonly string[];
};

export function ContextStudySelector({ studies }: { studies: ContextStudyVisual[] }) {
  ...
  return (
    <div>
        ...
        {studies.map((study) => {
          ...
          return (
            <button
              ...
              type="button"
              role="tab"
              id={`context-tab-${study.id}`}
              aria-selected={selected}
              aria-controls={`context-panel-${study.id}`}
      ...
      <div
        ...
        aria-labelledby={`context-tab-${active.id}`}
        className="space-y-8"
      >
        <div>
          ...
          <p className="mt-3 text-xs text-stone-500">
            Context study — conceptual proposal renderings, not confirmed installation views.
          </p>
        </div>
        {active.images.map((img) => (
          <ProposalFigure key={img.id} media={img} />
        ))}
