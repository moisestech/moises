...
import type { NvBudgetLine, NvPlaceholderMedia } from '@/content/grants/no-vacancy-2026/volver-a-valer';

export function NvSection({
  id,
  title,
  children,
  className = '',
...
export function NvPlaceholderFigure({
  media,
...
export function NvBudgetTable({ lines, total }: { lines: NvBudgetLine[]; total: number }) {
  return (
...
export function NvExperienceBeats({
  beats,
}: {
...
export function NvVisitorJourney({ steps }: { steps: readonly string[] }) {
  return (
...
export function NvRelatedWorks({
  works,
}: {
  works: readonly {
    slug: string;
