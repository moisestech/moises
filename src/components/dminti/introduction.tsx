...
export function Introduction() {
  ...
  const renderArtistLink = (artist: Artist) => (
    <a
      href={`https://instagram.com/${artist.handle.replace('@', '')}`}
      target="_blank"
      rel="noopener noreferrer"
      className={`inline-block px-1 rounded-sm text-sm transition-all duration-300 cursor-pointer ${
        theme === 'dark' 
  ...
  const renderPartnerLink = (partner: Partner) => (
    <a
      href={partner.url}
      target="_blank"
      rel="noopener noreferrer"
  ...
  return (
    <section className={`relative w-full py-20 ${theme === 'dark' ? 'bg-black/95' : 'bg-white/95'} backdrop-blur-sm`} id="introduction">
      <div className="w-full max-w-4xl px-4 mx-auto">
        <GlassmorphicPanel className={`p-8 mb-24 border ${theme === 'dark' ? 'border-white/10 bg-black/20' : 'border-black/10 bg-white/20'}`}>
          ...
          >
            <h2 className="text-4xl md:text-5xl font-bold mb-6 bg-clip-text text-transparent bg-gradient-to-r from-[#4D9DE0] via-[#E041B5] to-[#41E079]">
              Introduction
            </h2>
            <p className={`text-lg md:text-xl ${theme === 'dark' ? 'text-white/90' : 'text-black/90'} max-w-3xl mx-auto`}>
              A culminating, one-night-only digital art exhibition celebrating the creative impact of The Kaleidoscope (...Bigger) House by Laurie Simmons and Peter M. Wheelwright.
            </p>
          </motion.div>
          <motion.div
            initial={{ opacity: 0, y: 20 }}
